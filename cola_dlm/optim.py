...
Embeddings and scalars use AdamW; matrix (2D) parameters use Muon.
Two versions: MuonAdamW (single GPU), DistMuonAdamW (distributed).

Adapted from nanochat (https://github.com/karpathy/nanochat),
which is itself adapted from modded-nanogpt.
"""
...
# Compute dtype detection (simplified from nanochat/common.py)
_DTYPE_MAP = {"bfloat16": torch.bfloat16, "float16": torch.float16, "float32": torch.float32}

def _detect_compute_dtype():
    env = os.environ.get("COLA_DTYPE")
    if env is not None:
...
class MuonAdamW(torch.optim.Optimizer):
    ...
    Arguments:
        param_groups: List of dicts with 'kind' = 'adamw' or 'muon'.
            AdamW: 'lr', 'betas', 'eps', 'weight_decay'
            Muon: 'lr', 'momentum', 'ns_steps', 'beta2', 'weight_decay'
    """
...
# ---------------------------------------------------------------------------
# Distributed optimizer (multi-GPU, replaces DDP)
# ---------------------------------------------------------------------------

class DistMuonAdamW(torch.optim.Optimizer):
    """
    Distributed Muon+AdamW. Handles gradient sync internally (no DDP wrapper needed).

    Uses 3-phase async communication:
    1. Launch all reduce_scatter/all_reduce ops
    2. Wait, compute updates, launch all_gather
    3. Wait for gathers, copy back
    """
